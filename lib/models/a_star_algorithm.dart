...
class AStarAlgorithm {
  List<PathNode> findPath(PathNode start, PathNode goal) {
    debugPrint(
      '[A*] START: ${start.id} (${start.x}, ${start.y}) '
      '-> GOAL: ${goal.id} (${goal.x}, ${goal.y})',
    );
    // f(n) = g(n) + h(n)
    ...
    while (openSet.isNotEmpty) {
      ...
      if (current.id == goal.id) {
        ...
        debugPrint('[A*] Total path weight: ${totalWeight.toStringAsFixed(2)}');
        debugPrint(
          '[A*] Straight-line distance: ${straightLine.toStringAsFixed(2)}',
        );
        debugPrint(
          '[A*] Ratio (path/straight): '
          '${(totalWeight / straightLine).toStringAsFixed(2)}',
        );
        // ---
        return path;
      }
    ...
    debugPrint('[A*] NO PATH FOUND from ${start.id} to ${goal.id}');
