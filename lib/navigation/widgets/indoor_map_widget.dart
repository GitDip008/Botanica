...
class _IndoorMapWidgetState extends ConsumerState<IndoorMapWidget>
  ...
  /// House letter for the target section. Plant sections look like "G-HA" →
  /// house "A"; a bare room label like "A12" → "A".
  String? _houseLetter(String? section) {
    if (section == null) return null;
    final s = section.trim().toUpperCase();
    final gh = RegExp(r'^G-?H([A-Z])').firstMatch(s);
    if (gh != null) return gh.group(1);
    final bare = RegExp(r'^([A-Z])').firstMatch(s);
    return bare?.group(1);
  }

  /// House letter for a floor-plan room label ("A12" → "A").
  String? _roomHouse(String label) =>
      RegExp(r'^([A-Za-z])').firstMatch(label.trim())?.group(1)?.toUpperCase();

  /// On open, zoom the camera to the target house so its rooms fill the view.
  void _zoomToHouse(IndoorMapData data) {
    final house = _houseLetter(widget.activeSection);
    if (house == null) return;
    final pts = data.sectionPoints
        .where((sp) => _roomHouse(sp.label) == house)
        .map((sp) => sp.position)
        .toList();
    if (pts.isEmpty) return;
    if (pts.length == 1) {
      _mapController.move(pts.first, data.bounds.fitZoom + 1); // ~2x
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    final data = _mapData!;
    final targetHouse = _houseLetter(widget.activeSection);
    // "You are here" from the nearest known beacon (null until beacons wired).
          ...
          children: [
            ...
            MarkerLayer(
              markers: data.sectionPoints.map((sp) {
                // Glow every room in the target house (data is house-level).
                final isActive =
                    targetHouse != null && _roomHouse(sp.label) == targetHouse;
                // "You are here" = the exact room of the strongest beacon.
                final isHere = currentSection != null &&
                    sp.label.toUpperCase() == currentSection.toUpperCase();
                final plants = widget.sectionPlantMap?[sp.label] ?? [];
                final hasPlants = plants.isNotEmpty;
                return Marker(
                  point: sp.position,
                  width: (isActive || isHere) ? 44 : 36,
                  height: (isActive || isHere) ? 22 : 18,
                  child: GestureDetector(
                    onTap: hasPlants
                        ? () => widget.onSectionTapped?.call(sp.label, plants)
                        : null,
                    child: isHere
                        ? _HereLabel(label: sp.label)
                        : isActive
                            ? AnimatedBuilder(
...
/// "You are here" marker (blue), from the nearest beacon.
class _HereLabel extends StatelessWidget {
  const _HereLabel({required this.label});
  final String label;
