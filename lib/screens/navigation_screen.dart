...
/// Path result from A* — ordered list of points
...
class _IndoorMapScreenState extends State<IndoorMapScreen> {
  ...
  // ── New state ──────────────────────────────────────────────
  MapPoint? _tappedPoint; // CAD coordinate of the tapped point
  Offset? _tappedPixel; // pixel position to show the popup
  bool _showTapPopup = false; // show/hide the start/end picker popup
  ...
  void _runAStar() {
    ...
    if (startX == null || startY == null || endX == null || endY == null) {
      setState(() => _statusMsg = 'Enter valid coordinates');
      return;
    }
  ...
  void _onMapTap(TapDownDetails details, Size widgetSize) {
    // Use _mapKey to get the InteractiveViewer's RenderBox
    final RenderBox? box =
        _mapKey.currentContext?.findRenderObject() as RenderBox?;
    if (box == null) return;

    // Convert global position to the InteractiveViewer's local position
    final localPos = box.globalToLocal(details.globalPosition);

    // Inverse transform back to original coordinates before zoom/pan
    ...
    setState(() {
      _tappedPoint = cad;
      _tappedPixel =
          details.localPosition; // keep original localPosition to place the popup correctly
      _showTapPopup = true;
    });
  }
  ...
  void _setAsStart() {
    ...
    setState(() {
      ...
      _showTapPopup = false;
      // Open the input panel so the user sees it filled
  ...
  Widget _buildTapPopup(Size widgetSize) {
    ...
    // Compute popup position — above the tap point, centered on X
...
// ============================================================
// PATH PAINTER — draw the A* route on canvas
// ============================================================
